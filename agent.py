...
class CurrencyTemplate:
    def __init__(self):
        self.system_template = """
        You are a currency agent who show the user the current currency for a given country.

        The user's request will be denoted by four hashtags. Get the user's location request
        and return the current currency for that country.

        Retrieve the currency as a JSON Object, with the currency code, name, symbol and
        value for four other currency (north american dollar, euro, brazilian real and british pound). 
...
class Agent:
    ...
    def get_itinerary(self, request):
        ...
        currency_agent = LLMChain(
            llm=self.chat_model,
            prompt=currency_template.chat_prompt,
            verbose=self.verbose,
